...
export default function ScalpTerminal2() {
  ...
  useEffect(() => {
    async function loadMarket() {
      ...
      const res = await fetch("/api/gamma/search?q=bitcoin", {
        cache: "no-store",
      });
  ...
  if (loading || !market) {
    return (
      <div className="p-4 text-zinc-400 text-sm">Loading scalp terminal…</div>
    );
  }
  ...
  return (
    <div className="w-full bg-zinc-900 border h-full border-zinc-700 rounded-md overflow-hidden font-mono text-xs">
      <div className="px-3 py-2 border-b border-zinc-700">
        <div className="text-zinc-300">{market.question ?? market.slug}</div>
          ...
          {bids.map((l) => (
            <div
              ...
              className={clsx(
                ...
                counter && "bg-green-500/30"
              )}
            >
              <span className="text-green-400">{l.price.toFixed(2)}</span>
              <span className="text-zinc-400">{l.size}</span>

              {counter && (
                <span className="absolute right-1 text-green-300">●</span>
              )}
            </div>
          ))}
        </div>
          ...
          {asks.map((l) => (
            <div
              ...
              className={clsx(
                "relative flex justify-between px-2 py-[2px] cursor-pointer",
                "hover:bg-red-500/10",
                counter && "bg-red-500/30"
              )}
            >
              <span className="text-red-400">{l.price.toFixed(2)}</span>
              <span className="text-zinc-400">{l.size}</span>

              {counter && (
                <span className="absolute right-1 text-red-300">●</span>
              )}
            </div>
          ))}
        </div>

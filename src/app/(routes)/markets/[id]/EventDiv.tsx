...
export default function EventDiv({ markets, series }: { markets: Market[]; series: any[] }) {
  ...
  return (
    <div className="flex h-full items-center justify-center w-full">
      <div className="border w-[80vw] h-[40vw] flex shadow-lg">
        ...
        {/* правая колонка — order window + smth */}
          ...
          <div className="w-full h-[60%] w-[50%] border rounded-[40px] items-center flex justify-center p-5">
            {selected.market ? (
              <div className="flex flex-col h-full gap-3 p-5 w-[80%]">
                <p className="text-lg font-semibold">{selected.market.question}</p>
                <div className="flex justify-between gap-3 w-full">
                  <button
                    onClick={() => setSelected(s => ({ ...s, side: 'yes' }))}
                    className={`px-4 py-1.5 rounded-[10px] text-white transition ${
                      selected.side === 'yes' ? 'bg-green-600 ring-2 ring-green-300' : 'bg-green-500 hover:bg-green-400'
                    }`}
                  >YES</button>
                  <button
                    onClick={() => setSelected(s => ({ ...s, side: 'no' }))}
                    className={`px-4 py-1.5 rounded-[10px] text-white transition ${
                      selected.side === 'no' ? 'bg-red-600 ring-2 ring-red-300' : 'bg-red-500 hover:bg-red-400'
                    }`}
                  >NO</button>
                </div>
                {selected.side && (
